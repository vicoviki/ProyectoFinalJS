...
async function contarProductosCarrito() {
    let count = carrito.reduce((acc, p) => (acc += p.cantidad), 0);
    const carritoCounter = document.getElementById('contador-carrito');
    carritoCounter.innerHTML = `<h6>${count}</h6>`
...
async function pintarCards() {
    ...
    const productosStock = await pintarJson(); 
    ...
    productosStock.forEach((producto) => {
        ...
        div.innerHTML = `
            <div class="card col-sm-12 col-md-6 col-lg-4 col-xl-4 mb-2 me-4" style="width:20rem">
